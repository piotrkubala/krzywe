...
std::vector <sf::Vertex> prosta(double a, double b, double c, double t0, double tk, int n)
{
    std::vector <sf::Vertex> line;
    float dt = (tk - t0) / n, t;

    line.push_back(sf::Vertex(sf::Vector2f(a * t0 + c, b * t0 + c)));
    line.push_back(sf::Vertex(sf::Vector2f(a * t0 + c, b * t0 + c)));

    for(int i = 2; i <= n + 2; i++)
    {
        t = (t0 + dt * i);
        line.push_back(sf::Vertex(sf::Vector2f(a * t + c, b * t + c)));
